...
describe.skip('ImImageSlidesComponent', () => {
  ...
  it('should show no slides', () => {
    spectator = createHost(`<im-image-slides [photoUrls]="[]">im-image-slides content</im-image-slides>`);
    expect(spectator.queryAll('ion-slide').length).toBe(0);
  });

  it('should show 3 slides', () => {
    spectator = createHost(
      `<im-image-slides [photoUrls]="['','','']">im-image-slides content</im-image-slides>`
    );
