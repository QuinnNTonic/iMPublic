...
export const OfferReducer = createReducer(
  initialState,
  on(OffersActions.refreshOffersForProfile, (state, { profile }): OffersState => {
    return {
      ...state,
      profileLoaded: state.profileLoaded.filter((p) => p !== profile.id),
    };
  }),
  on(OffersActions.loadOffersForProfileSuccess, (state, { offers, profileId }): OffersState => {
    return {
      ...state,
  ...
  on(OffersActions.createOfferSuccess, (state, { offer }): OffersState => {
    return {
      ...state,
      offers: offersAdapter.upsertOne(offer, state.offers),
    };
  }),
  on(OffersActions.updateOfferSuccess, (state, { offer }): OffersState => {
    return {
      ...state,
      offers: offersAdapter.upsertOne(offer, state.offers),
    };
  }),
  on(OffersActions.deleteOfferSuccess, (state, { deletedId }): OffersState => {
    return {
      ...state,
      offers: offersAdapter.removeOne(deletedId, state.offers),
    };
  }),
  on(OffersActions.uploadImagesSuccess, (state, { offer }): OffersState => {
    return {
      ...state,
      offers: offersAdapter.upsertOne(offer, state.offers),
    };
  }),
  on(OffersActions.deleteImageSuccess, (state, { offer }): OffersState => {
    return {
      ...state,
      offers: offersAdapter.upsertOne(offer, state.offers),
    };
